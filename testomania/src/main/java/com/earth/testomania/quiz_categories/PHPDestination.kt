...
@Destination(route = ROUTE_PHP)
@Composable
fun PHPQuiz(navigator: DestinationsNavigator) {
    val viewModel: PHPViewModel = hiltViewModel()
    MainQuizScreen(navigator, viewModel)
}

class GetPHPUseCse @Inject constructor(private val repository: QuizRepository) :
    QuizApiBaseUrlUseCase() {

    override suspend fun getRepResult() = repository.getQuizList(QuizApiCategory.PHP)
