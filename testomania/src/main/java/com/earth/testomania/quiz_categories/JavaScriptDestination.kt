...
const val ROUTE_JAVASCRIPT = "home/javascript"

@Destination(route = ROUTE_JAVASCRIPT)
@Composable
fun JavaScriptQuiz(navigator: DestinationsNavigator) {
    val viewModel: JavaScriptViewModel = hiltViewModel()
    MainQuizScreen(navigator, viewModel)
}

class GetJavaScriptUseCse @Inject constructor(private val repository: QuizRepository) :
    QuizApiBaseUrlUseCase() {

    override suspend fun getRepResult() = repository.getQuizList(QuizApiCategory.JavaScript)

}

@HiltViewModel
class JavaScriptViewModel @Inject constructor(
    useCase: GetJavaScriptUseCse,
