...
const val ROUTE_WORDPRESS = "home/wordpress"

@Destination(route = ROUTE_WORDPRESS)
@Composable
fun WordPressQuiz(navigator: DestinationsNavigator) {
    val viewModel: WordPressViewModel = hiltViewModel()
    MainQuizScreen(navigator, viewModel)
}

class GetWordPressUseCse @Inject constructor(private val repository: QuizRepository) :
    QuizApiBaseUrlUseCase() {

    override suspend fun getRepResult() = repository.getQuizList(QuizApiCategory.WordPress)

}

@HiltViewModel
class WordPressViewModel @Inject constructor(
    useCase: GetWordPressUseCse,
