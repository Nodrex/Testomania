...
const val ROUTE_KUBERNETES = "home/kubernetes"

@Destination(route = ROUTE_KUBERNETES)
@Composable
fun KubernetesQuiz(navigator: DestinationsNavigator) {
    val viewModel: KubernetesViewModel = hiltViewModel()
    MainQuizScreen(navigator, viewModel)
}

class GetKubernetesUseCse @Inject constructor(private val repository: QuizRepository) :
    QuizApiBaseUrlUseCase() {

    override suspend fun getRepResult() = repository.getQuizList(QuizApiCategory.Kubernetes)

}

@HiltViewModel
class KubernetesViewModel @Inject constructor(
    useCase: GetKubernetesUseCse,
